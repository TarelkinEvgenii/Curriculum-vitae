...
def main(path_to_file: str, path_to_save: str) -> None:
    """Функция для нахождения важных строк в файле фич bureau."""
    ...
    bureau_copy = bureau.copy(deep=True)

    columns_for_tests = list(bureau_copy.columns)
    ...
    bureau = bureau[important_columns]
    bureau.to_csv(
        os.path.join(path_to_save, 'important_bureau_train.csv'),
        index=False,
    )
